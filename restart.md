...
4. `<Route caseSensitive>` 属性用于指定：匹配时是否**区分大小写**（默认为 false）。
...
`end` 当NavLink上添加了end属性后，若Home的子组件匹配成功，则Home的导航没有高亮效果
...
👉 v6这边做了更改，动态修改className需要给className提供一个回调函数，没有activeClassName了
...
1. 作用：**只要`<Navigate>`组件被渲染，就会修改路径，切换视图**❗️
      ...
      <div>
        <ul className="nav nav-tabs">
          <li>
            ...
            {/* <NavLink to="/news" className="list-group-item">News</NavLink> */} // ✖️ 这样就说明是根标签了
            <NavLink to="news" className="list-group-item">News</NavLink> // 可以省略一级的标签，但是不能加斜杠
          </li>
          <li>
...
- 路由链接中的 **`to`** 属性值，可以是
  ...
  - **`to="./news"`**，即相对路径，在不破坏当前路径的前提下，加上后续的路由
  - **`to="news"`**
...
不管是否是路由组件，只要被BrowserRouter组件包裹就返回true

👉 第三方组件的封装者，需要知道使用者是否在路由环境下使用这个组件的场景
...
<img src="restart.assets/image-20240406235517757.png" alt="image-20240406235517757" style="zoom:50%;" />
